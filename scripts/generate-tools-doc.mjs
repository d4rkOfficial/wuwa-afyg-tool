...
function main() {
    ...
    lines.push('| `magnetic_follow` | 磁力光标跟手性 | 50-400（ms，越小越跟手） |')
    lines.push('| `magnetic_sensitivity` | 磁力光标灵敏度 | 0.05-0.3（越大越跟手） |')
    lines.push('| `magnetic_spin` | 磁力光标旋转速度 | 4-30（s/圈，越小越快） |')
    lines.push('| `magnetic_wobble` | 磁力光标吸附晃动强度 | 0-10（0=关闭） |')
    lines.push('| `magnetic_border` | 磁力光标内部描边粗细 | 0.5-3（px，四角边框固定 2px） |')
