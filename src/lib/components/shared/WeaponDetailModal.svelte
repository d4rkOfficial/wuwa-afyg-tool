<script lang="ts">
    ...
    let data = $state<WeaponInfo | null>(null)
            ...
            <div class="flex-1 overflow-y-auto px-5 py-4">
                ...
                {:else if data}
                    <div class="space-y-6">
                        ...
                        <section>
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-500">属性</h3>
                            <div class="grid grid-cols-2 gap-3">
                                <div class="rounded-lg bg-zinc-800/60 p-3 text-center">
                                    <div class="text-[11px] text-zinc-500">Lv90 基础攻击</div>
                                    <div class="mt-1 text-sm font-semibold text-zinc-200">{data.lv90BaseAtk}</div>
                                </div>
                                <div class="rounded-lg bg-zinc-800/60 p-3 text-center">
                                    <div class="text-[11px] text-zinc-500">{data.substat.name}</div>
                                    <div class="mt-1 text-sm font-semibold text-zinc-200">{data.substat.value}</div>
                                </div>
                            </div>
                        </section>

                        <section>
                            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-500">武器效果</h3>
                            <div class="rounded-lg border border-zinc-800 bg-zinc-900/60 px-4 py-3">
                                <div class="text-sm font-medium text-zinc-200">{data.effect.name}</div>
                                <div class="mt-2 text-xs text-zinc-400 leading-relaxed">
                                    {@html renderDesc(data.effect.desc)}
                                </div>
                            </div>
                        </section>
                    </div>
