...
>
    ...
    {:else if !lookupCompactHidden}
        <div class="min-h-0 flex-1 space-y-1 overflow-y-auto px-2 pt-2">
            {#each grouped.groups as group, i (group.key)}
                ...
                <div
                    class={[
                        'overflow-hidden rounded-none transition-colors',
                        ...
                        <div transition:slide|local={{ duration: 200 }} class="space-y-1 pb-1">
                            {#each group.projects as project (project.id)}
                                ...
                                >
                                    {#if compact}
                                        <span
                                            class="flex size-6 items-center justify-center rounded-none bg-(--theme-sidebar-text)/10 text-xs"
                                            >{project.name[0]}</span
                                        >
                                    {:else}
    ...
    <!-- 底部操作区：新建 / 从本地导入 / 从工坊下载（可在 设置-交互相关 关闭） -->
        ...
        <div class="shrink-0 border-t px-2 pt-2 pb-3 space-y-0.5" style="border-color: var(--theme-divider-border)">
            ...
            {:else}
                <button
                    ...
                    class="flex w-full shrink-0 items-center justify-center rounded-none py-1 text-(--theme-sidebar-text)/40 transition-colors hover:bg-(--theme-sidebar-text)/5 hover:text-(--theme-sidebar-text)/80"
