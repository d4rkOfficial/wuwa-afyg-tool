...
>
            ...
            {#each project!.team as slot, ci (ci)}
                ...
                >
                    ...
                    <div class="flex shrink-0 items-center gap-2.5">
                        ...
                        {:else}
                            <span
                                class="flex size-10 shrink-0 items-center justify-center rounded-full bg-(--theme-input-bg) text-xs text-(--theme-muted-text)"
                                >{character?.slice(0, 1) ?? ci + 1}</span
                            >
                        {/if}
                        <div class="min-w-0 flex-1">
                            <div class="truncate text-sm font-semibold text-(--theme-modal-text)">
                                {character ?? `空位 ${ci + 1}`}
                            </div>
                            <div class="text-[11px] text-(--theme-muted-text)">{slotLabelOf(character ?? '')}</div>
                        </div>
                    </div>

                    {#if !character}
                        <div class="py-6 text-center text-xs text-(--theme-muted-text)">该位次没有角色</div>
                    {:else}
                        ...
                        <div class="theme-scrollbar -mr-1.5 min-h-0 flex-1 space-y-2.5 overflow-y-auto pr-1.5">
                            ...
                            {#if planRowsFor(character).length === 0}
                                <div class="py-6 text-center text-xs text-(--theme-muted-text)">正在加载角色数据…</div>
                            {/if}
                        </div>
                    {/if}
                </div>
    ...
    {:else}
        ...
        <div class="flex h-full min-h-0 flex-col">
            <p class="mb-3 shrink-0 text-sm text-(--theme-muted-text)">
            ...
            <div class="flex min-h-0 flex-1 gap-4">
                    ...
                    <div class="theme-scrollbar min-h-0 flex-1 space-y-1 overflow-y-auto pr-0.5">
                        {#each homeRows as row (row.name)}
                            ...
                            >
                                ...
                                {:else}
                                    <span
                                        class="flex size-8 shrink-0 items-center justify-center rounded-full bg-(--theme-input-bg) text-[10px] text-(--theme-muted-text)"
                        ...
                        {#if homeRows.length === 0}
                            <div class="py-8 text-center text-xs text-(--theme-muted-text)">没有匹配的角色</div>
                        {/if}
                    </div>
                </div>
                ...
                <div class="theme-scrollbar min-h-0 flex-1 space-y-2.5 overflow-y-auto pr-0.5">
                    {#if !selected}
                        <div
                            class="flex h-full flex-col items-center justify-center gap-2 text-sm text-(--theme-muted-text)"
                        >
                    ...
                    {:else}
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-semibold text-(--theme-modal-text)">{selected}</span>
                        ...
                        {#if planRowsFor(selected).length === 0}
                            <div class="py-8 text-center text-xs text-(--theme-muted-text)">正在加载角色数据…</div>
                        {/if}
                    {/if}
                </div>
            </div>
