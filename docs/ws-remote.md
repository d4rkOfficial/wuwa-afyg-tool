...
椰果工具箱支持通过 URL hash 连接用户指定的 WebSocket 服务器，由服务器**接管操作**：AI 助手能做什么、能查什么，WS 服务器就能做什么、查什么（两者共用同一套 82 个工具注册表与执行引擎）。
...
82 个工具，与 AI 助手完全一致，按领域分组（可用 `node scripts/ws-demo-server.mjs` 后输入 `tools` 查看完整清单，或参考 `src/lib/ai/tools/`）：
